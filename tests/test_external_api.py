...
@patch.dict("os.environ", {"API_KEY": "NTP0jsKdybTUyjG8QerQGzzRft0fiuhc"})
@patch("requests.request")
def test_convert_exchange_rate(mock_request):
    print(f"API key inside function: {api_key}")
    # Функкция тестирования Подготавливаем данные для теста
    tr_list = {"operationAmount": {"currency": {"code": "USD"}, "amount": "100"}}
    ...
    mock_response.json.return_value = {
        "result": 7000
    }  # Предположим, что 100 USD = 7000 RUB
    ...
    mock_request.assert_called_once_with(
        "GET", expected_url, headers={"apikey": api_key}
    )
